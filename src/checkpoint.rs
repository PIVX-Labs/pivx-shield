//Return an array of satic ordered checkpoints
fn get_checkpoint(block_height: i32, isTestnet: bool) -> (i32, String) {
    let test_network_checkpoins = [(1125777, "")];
    let main_network_checkpoins = [(0, "0")];
    let used_checkpoints = if isTestnet {
        test_network_checkpoins
    } else {
        main_network_checkpoins
    };

    let mut found_checkpoint: (i32, String) =
        (used_checkpoints[0].0, String::from(used_checkpoints[0].1));
    let mut min_dist = i32::MAX;

    for checkpoint in used_checkpoints {
        if i32::abs(checkpoint.0 - block_height) < min_dist {
            found_checkpoint = (checkpoint.0, String::from(checkpoint.1));
            min_dist = i32::abs(checkpoint.0 - block_height);
        }
    }
    return found_checkpoint;
}
