...
use pivx_primitives::consensus::{
    BlockHeight, Network, NetworkConstants, Parameters, TEST_NETWORK,
};
use pivx_primitives::zip32::Scope;
use sapling::value::NoteValue;
use sapling::Node;
use sapling::Note;
use sapling::Rseed::BeforeZip212;
use std::error::Error;
use std::io::Cursor;
...
fn check_tx_decryption() {
    ...
    let key = UnifiedFullViewingKey::from_sapling_extended_full_viewing_key(
...
pub async fn test_create_transaction() -> Result<(), Box<dyn Error>> {
    ...
    let mut commitment_tree = read_commitment_tree::<Node, _, DEPTH>(commitment_tree)?;
    let address =
