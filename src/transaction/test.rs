...
use pivx_client_backend::encoding::decode_extended_full_viewing_key;
...
impl Default for RngTest {
    fn default() -> Self {
	Self { seed: Wrapping(123) }
    }
}

impl RngCore for RngTest {
    fn next_u32(&mut self) -> u32 {
	self.seed *= 134314134;
	self.seed.0 as u32
    }
    fn next_u64(&mut self) -> u64 {
	self.seed *= 3180413498;
	self.seed.0
    }
...
fn check_tx_decryption() {
    ...
    let (nullifiers, comp_note) = handle_transaction_internal(&mut tree, tx, &key, true).unwrap();
    //This was a t-s tx
...
pub fn test_create_transaction() -> Result<(), Box<dyn Error>> {
    ...
    let address = "ptestsapling1nfg50r63hd4u5xn0jt2rsw52jlndtqqt87azvaflh25j4kcw2p0ann4x7ae5x8yfqvyukhu2yxx";
    ...
    let key = UnifiedFullViewingKey::new(Some(extended_spending_key.to_diversifiable_full_viewing_key()), None).ok_or("Failed to construct key")?;
    ...
    let (_nullifiers, notes) = handle_transaction_internal(&mut commitment_tree, input_tx, &key, true)?;
